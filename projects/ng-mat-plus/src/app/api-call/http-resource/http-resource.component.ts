...
import { IApiResponse } from 'ng-mat-plus/interface';
...
export class ResourseComponent {
  ...
  users = this.$httpService.getResource<DummyUsersResponse>(
    ...
    getMultipleApis(): any {
    return this.$httpService.getMultipleApis<
      [
        IApiResponse<IUnit[]>,
        IApiResponse<IFrequency[]>,
        IApiResponse<IRoute[]>,
        IApiResponse<IDuration[]>,
        IApiResponse<IDirection[]>,
        IApiResponse<IUnitCode[]>, // interventionTypes
        IApiResponse<IUnitCode[]>  // interventionNatures
      ]
    >([
      `${'dd'}/units`,
      `${'dd'}/frequencies`,
      ...
      `${'dd'}/durations`,
      `${'dd'}/directions`,
      `${'dd'}/intervention-types`,
      `${'dd'}/intervention-natures`
    ])
  }
  ...
  singleUserResource = this.$httpService.httpResource<SingleUserResponse>(() =>
    (
      {
        url: `https://dummyjson.com/users/${this.selectedUserId()}`
      }
    ),
    {
