...
import {{ '{' }} LibDataTransferService {{ '}' }} from 'ng-mat-plus/services';
...
{{ '}' }}
  ...
  <pre class="overflow-x-auto p-1 bg-gray-800 rounded">
    <code class="text-sm text-green-300">
import {{ '{' }} LibDataTransferSignalService {{ '}' }} from 'ng-mat-plus/services';
...
export class DataTransferComponent implements OnDestroy {
  ...
  constructor(private $dataTransferService: LibDataTransferService) {
    ...
    this.$dataTransferService
      .getDataObservable<IUser[]>('userProfile')
      .pipe(takeUntil(this._destroy))
      .subscribe((user: IUser[] | null) => {
        if (user) {
          this.userProfile = user;
      ...
      this.$dataService.setData<IUser[]>('employees', [
        { name: 'Alice', age: 20 },
        { name: 'Bob', age: 50 }
      ]);
  }
...
// =============================================================================
  ...
  usersSignal = this.$dataService.getDataSignal<IUser[]>('users');

  addUser() {
    this.$dataService.addItem<IUser>('users', { name: 'John Doe', age: 30 });
  }

  updateFirstUser() {
    this.$dataService.updateItem<IUser>('users', 0, { name: 'Jane Smith', age: 28 });
  }

  removeFirstUser() {
    this.$dataService.removeItem<IUser>('users', 0);
  }
}
